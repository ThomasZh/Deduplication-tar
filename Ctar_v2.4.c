...
struct arch_header *create_arch(struct arch_header *node, struct arch_header *temp, char *arch_name)
{
	...
	struct arch_header *test;
	test = (struct arch_header *)malloc(sizeof(struct arch_header));

	arch_fd = open(arch_name, O_RDONLY);

	read(arch_fd, test, sizeof(struct arch_header));

	printf("1.%s|%d|%d\t",test->name, 5000, test->duplicateLocationIndex[5000]);

	lseek(arch_fd, test->file_info.st_size, SEEK_CUR);

	read(arch_fd, test, sizeof(struct arch_header));

	printf("2.%s|%d|%d\n", test->name, 5000, test->duplicateLocationIndex[5000]);
	close(arch_fd);
	return node;
}
...
void release_arch(char *arch_name)
{
	...
	/*header_read = read(arch_fd, head, sizeof(struct arch_header));

	printf("1.%s|%d|%d\t", head->name, 5000, head->duplicateLocationIndex[5000]);

	lseek(arch_fd, head->file_info.st_size, SEEK_CUR);

	read(arch_fd, head, sizeof(struct arch_header));

	printf("2.%s|%d|%d\n", head->name, 5000, head->duplicateLocationIndex[5000]);*/
	

	while (x<2)
	{
		header_read = read(arch_fd, head, sizeof(struct arch_header));
		printf("%s, %d\n", head->name, head->duplicateLocationIndex[1]);
		if (head->duplicateLocationIndex[1] == 0) //중복이 없는 파일풀기
		{
			//release_fd = open(head->name, O_WRONLY | O_TRUNC | O_CREAT, 0664);

			release_fd = open("1st.txt", O_WRONLY | O_TRUNC | O_CREAT, 0664);
		...
		else if(head->duplicateLocationIndex[1] != 0) 
		{	//중복이 있는 파일의 풀기, 복원하는 과정에서 몇번째 있는 파일의 중복 데이터를 복사할것지 판단이 필요
			//release_fd = open(head->name, O_WRONLY | O_TRUNC | O_CREAT, 0664);
			release_fd = open("2nd.txt", O_WRONLY | O_TRUNC | O_CREAT, 0664);

			lseek(arch_fd, 0, SEEK_SET);
			
			int prev_read = read(arch_fd, prev, sizeof(struct arch_header));
			int loc = lseek(arch_fd, 0, SEEK_CUR);
			printf("%d\n", loc);
			sumlen = prev_read;
			...
			for (i = 0; i < head->chunks; i++)
			{
				for (check = 0; check < prev->chunks; check++)
				{
						...
						&& head->duplicateLocationIndexSize[i] == (prev->cut[check + 1] - prev->cut[check]))
					{
						...
						y = i;
						//printf("|%d %d| ", head->duplicateLocationIndexSize[i], prev->cut[check + 1] - prev->cut[check]);
					}
				}
			}
			
			printf("%d\n", y);
			for (i = 0; i < head->chunks - y; i++)
			{
				lseek(arch_fd, sumlen, SEEK_SET);
