...
int main(int argc, char **argv)
{
	...
	{
		arch_name = (char *)malloc(sizeof(char *) * strlen("custom_VLC.tar") + 1);
		strcpy(arch_name, "custom_VLC.tar");
	}
...
struct arch_header *create_arch(struct arch_header *node, struct arch_header *temp, char *arch_name)
{
	int arch_fd, fd, i, j, c, check = 0, bitcut, match = 0, unmatch = 0, remaining, p;
	void *buf;
	...
	else
	{
		...
		if (hashnode->next == NULL) //두번째파일 해쉬 값 비교
		{
			for (i = 0; i < temp->chunks; i++)
			{
				...
				{
					lseek(fd, temp->cut[i], SEEK_SET);
					read(fd, buf, temp->cut[i + 1] - temp->cut[i]);
					write(arch_fd, name, strlen(name));
					write(arch_fd, buf, temp->cut[i + 1] - temp->cut[i]);

					unmatch++;
				}
			}
			printf("check : %d | unmatch : %d\n", check, unmatch);
