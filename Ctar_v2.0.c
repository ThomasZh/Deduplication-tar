/*SHA1을 이용하여 해시값 비교를 통해 100%중복이 되는 파일을 제거*/
...
int main(int argc, char **argv)
{
	...
	{
		arch_name = (char *)malloc(sizeof(char *) * strlen("custom(SHA1_O).tar") +1);
		strcpy(arch_name, "custom(SHA1_O).tar");
	}
...
struct arch_header *create_arch(struct arch_header *node, struct arch_header *temp, char *arch_name)
{
	...
	struct arch_header *hashnode = node;
	SHA1Context sha;
	int i, match = 0;
	unsigned hash[5];
	...
	while ((len_read = read(fd, buf, temp->file_info.st_size)) > 0)
	{
		...
		else
		{
			...
			{
				while (current->next != NULL) //세번째 파일 해쉬값 비교
				{
